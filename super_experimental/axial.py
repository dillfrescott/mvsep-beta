...
class NeuralModel(nn.Module):
    def __init__(self, in_channels=2, out_channels=2, hidden_channels=90, n_modes=(90, 90)):
        ...
        self.pos_emb = ContinuousAxialPositionalEmbedding(
            dim = hidden_channels,
            num_axial_dims = 2,
        )
    ...
    def forward(self, x):
        ...
        # Add positional embedding. Add a batch dimension *before* permuting.
        axial_pos_emb = self.pos_emb(x.shape[2:]).unsqueeze(0).permute(0, 3, 1, 2)
        x = x + axial_pos_emb
