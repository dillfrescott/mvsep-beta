...
class ProdigyComplex(Prodigy):
    def __init__(self, *args, **kwargs):
        ...
        self.defaults.setdefault('d_min', 1e-6)
        self.defaults.setdefault('amsgrad', False)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if grad.is_complex():
                    d_hat += torch.abs(grad).pow(2).sum()
                else:
                    d_hat += grad.pow(2).sum()

        d_hat = d_hat.sqrt()

        for group in self.param_groups:
            lr = group['lr']
            d = group.get('d', self.defaults['d'])
            d_max = group.get('d_max', self.defaults['d_max'])
            d_min = group.get('d_min', self.defaults['d_min'])
            k = group.get('k', self.defaults['k'])
            
            d = max(d, d_hat)
            
            if self.defaults.get('safeguard_warmup', False):
                d = max(d, d_min)
            
            lr_eff = lr / max(1, (d/d_max)**k)
            ...
            for p in group['params']:
                ...
                if grad.is_complex():
                    exp_avg_sq.mul_(beta2).addcmul_(torch.conj(grad), grad, value=1 - beta2)
                else:
                ...
                p.addcdiv_(exp_avg, denom, value=-lr_eff / bias_correction1)
...
def main():
    ...
    model = NeuralModel()
    optimizer = ProdigyComplex(model.parameters(), lr=1e-4)
