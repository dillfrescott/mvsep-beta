...
class QFRN(nn.Module):
    def __init__(self, ch, k=3):
        super().__init__()
        p, C = k//2, ch//2
        self.H = nn.Conv2d(C, 2*C, k, padding=p)
        self.hyper = nn.Conv2d(C, C, 1)
        self.couple = nn.Sequential(nn.Conv2d(ch, ch, 1), nn.SiLU(), nn.Conv2d(ch, ch, 1))
        self.global_couple = nn.Conv2d(C, C, 1)
        self.gate = nn.Sigmoid()
        
    def forward(self, x):
        B, D, H, W = x.shape; C = D//2
        Psi = torch.complex(x[:,:C], x[:,C:])
        Hr, Hi = self.H(Psi.real).chunk(2,1)
        H_local = torch.complex(Hr, Hi) * torch.sigmoid(self.hyper(Psi.abs()))
        V = self.couple(x)
        Vc = torch.complex(V[:,:C], V[:,C:])
        psi_mean = Psi.mean([2,3], True)
        global_couple_real = self.global_couple(psi_mean.real)
        global_couple_imag = self.global_couple(psi_mean.imag)
        global_couple_complex = torch.complex(global_couple_real, global_couple_imag)
        Psi_next = Psi + 1j*H_local + Vc + global_couple_complex
        amp = Psi_next.abs()
        gate = self.gate(amp / (amp.mean([1,2,3], True)+1e-6))
        Psi_next = gate * Psi_next / (amp.mean([1,2,3], True)+1e-6)
        return torch.cat([Psi_next.real, Psi_next.imag], 1)

class NeuralModel(nn.Module):
    def __init__(self, in_channels=2, sources=2, freq_bins=2049, embed_dim=1024, depth=12):
        super().__init__()
        ...
        self.model_layers = nn.ModuleList([QFRN(embed_dim) for _ in range(depth)])
    ...
    def forward(self, x_stft, x_audio):
        ...
        x = self.input_proj_stft(x_stft).unsqueeze(-1).permute(0, 2, 3, 1)
        for layer in self.model_layers: x = layer(x)
        x = torch.tanh(x).squeeze(2).permute(0, 2, 1)
