...
def inference(
    ...
    input_files = [
        f for f in os.listdir(input_dir) if f.lower().endswith((".wav", ".flac"))
    ]
    input_files.sort()
    ...
    for filename in input_files:
        ...
        with tqdm(
            total=math.ceil(total_length / step_size),
            ...
            for start in range(0, total_length, step_size):
                ...
                with torch.no_grad():
                    pred_output = model(
                        spec.unsqueeze(0), padded_chunk.unsqueeze(0)
                    ).squeeze(0)
                ...
                instrumental_spec = torch.stack(
                    [iL_cmask * spec[0], iR_cmask * spec[1]], dim=0
                )
                vocal_spec = torch.stack(
                    [vL_cmask * spec[0], vR_cmask * spec[1]], dim=0
                )

                vocal_chunk_padded = torch.istft(
                    vocal_spec,
                    n_fft=n_fft,
                    hop_length=hop_length,
                    window=window,
                ...
                if start > 0:
                    fade_window[:effective_overlap] = torch.linspace(
                        0, 1, effective_overlap, device=device
                    )

                if start + L < total_length:
                    fade_window[-effective_overlap:] = torch.linspace(
                        1, 0, effective_overlap, device=device
                    )
        ...
        torchaudio.save(
            output_vocal_path, vocals.cpu().clamp(-1.0, 1.0), sr, format="flac"
        )
        torchaudio.save(
            output_instrumental_path,
            instrumentals.cpu().clamp(-1.0, 1.0),
            sr,
            format="flac",
        )
...
def main():
    ...
    if args.infer:
        inference(
            model, args.checkpoint_path, args.input_dir, args.output_dir, device=device
        )
