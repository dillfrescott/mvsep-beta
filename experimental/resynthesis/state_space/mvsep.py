...
class NeuralModel(nn.Module):
    def __init__(self, in_channels=2, sources=2, freq_bins=2049,
        ...
        self.model = S5Block(embed_dim, embed_dim, True)
    ...
    def forward(self, x_stft, x_audio):
        ...
        current_T = x.shape[1]
        x = x.view(B, current_T, self.num_streams * 2, F).permute(0, 2, 3, 1)
        return x
...
def loss_fn(pred_output,
    ...
    pred_output_reshaped = _reshape_pred(pred_output, F_dim, T)
    ...
    target_vocal_mag = torch.log1p(torch.abs(target_vocal_spec))
    target_instr_mag = torch.log1p(torch.abs(target_instr_spec))

    mag_loss = F.l1_loss(pred_vocal_mag_log1p, target_vocal_mag) + F.l1_loss(pred_instr_mag_log1p, target_instr_mag)
    ...
    v_spec_pred = pred_vocal_mag_linear * torch.exp(1j * pred_vocal_phase)
    i_spec_pred = pred_instr_mag_linear * torch.exp(1j * pred_instr_phase)

    complex_spec_loss = F.mse_loss(v_spec_pred.real, target_vocal_spec.real) + \
                        F.mse_loss(v_spec_pred.imag, target_vocal_spec.imag) + \
                        F.mse_loss(i_spec_pred.real, target_instr_spec.real) + \
                        F.mse_loss(i_spec_pred.imag, target_instr_spec.imag)

    spectrogram_loss = mag_loss + 0.1 * complex_spec_loss
...
def main():
    ...
    parser.add_argument('--best', action='store_true', help='Use the best SI-SDR checkpoint for inference instead of the default highest step checkpoint.')
    ...
    if args.train:
        ...
        if not checkpoint_to_load:
            if args.best:
                ...
                checkpoint_to_load = find_latest_checkpoint('ckpts')
                if checkpoint_to_load:
                    print(f"No checkpoint specified. Automatically using highest step checkpoint: {checkpoint_to_load}")
                else:
