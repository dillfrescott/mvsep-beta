...
def inference(model, checkpoint_path, input_wav_path, output_instrumental_path,
    ...
    instrumental = torch.zeros_like(input_audio)
    ...
    with tqdm(total=num_chunks, desc="Processing audio") as pbar:
        for i in range(0, total_length - chunk_size + 1, chunk_size - overlap):
            ...
            if i == 0:
                instrumental[:, i:i + chunk_size] = instrumental_chunk
            else:
                ...
                instrumental[:, i:i + cross_fade_length] *= fade_out
                instrumental[:, i:i + cross_fade_length] += instrumental_chunk[:, :cross_fade_length]

            instrumental[:, i + cross_fade_length:i + chunk_size] = instrumental_chunk[:, cross_fade_length:]

            pbar.update(1)

    instrumental = torch.clamp(instrumental, -1.0, 1.0)

    torchaudio.save(output_instrumental_path, instrumental.cpu(), sr)
