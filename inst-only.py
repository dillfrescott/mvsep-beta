...
def loss_fn(pred_inst_mag, target_inst_mag, mixture_phase, window, n_fft, hop_length):
    ...
    pred_inst_audio = torch.istft(pred_inst_spec, n_fft=n_fft, hop_length=hop_length, window=window)
    target_inst_audio = torch.istft(target_inst_spec, n_fft=n_fft, hop_length=hop_length, window=window)
...
def train(model, dataloader, optimizer, scheduler, loss_fn, device, epochs, checkpoint_steps, args, checkpoint_path=None, window=None):
    ...
    for epoch in range(epochs):
        for batch in dataloader:
            # Unpack the batch
            mixture_mag, mixture_phase, inst_mag, _, _, _ = batch  # Only need mixture and instrumentals
            mixture_mag = mixture_mag.to(device)
            mixture_phase = mixture_phase.to(device)
            inst_mag = inst_mag.to(device)

            optimizer.zero_grad()

            # Forward pass (model now only outputs instrumentals)
            pred_inst_mag = model(mixture_mag)

            # Compute loss (only for instrumentals)
            loss = loss_fn(pred_inst_mag, inst_mag, mixture_phase, window, args.n_fft, args.hop_length)
...
def inference(model, checkpoint_path, input_wav_path, output_inst_path,
    ...
    with tqdm(total=num_chunks, desc="Processing audio") as pbar:
        for i in range(0, total_length - chunk_size + 1, chunk_size - overlap):
            ...
            with torch.no_grad():
                pred_inst_mag = model(chunk_mag)  # Model now only outputs instrumentals
    ...
    torchaudio.save(output_inst_path, instrumentals.cpu(), sr)
...
def main():
    ...
    parser.add_argument('--output_inst', type=str, default='output_inst.wav', help='Path to output instrumental WAV file')
    ...
    if args.train:
        ...
        if args.input_wav is None:
            ...
            return
        inference(model, args.checkpoint_path, args.input_wav, args.output_inst, device=device, n_fft=args.n_fft, hop_length=args.hop_length)
    else:
